...
landcoverKeys   = { wood="wood", forest="wood",
                    ...
                    fell="grass", grassland="grass", grass="grass", heath="grass", meadow="grass", allotments="grass", park="grass", village_green="grass", recreation_ground="grass", scrub="grass", shrubbery="grass", tundra="grass", garden="grass", golf_course="grass" }
...
function way_function()
	...
	-- Boundaries and parks within relations
	local admin_level = 11
	local isBoundary = false
	local rel_park_class = ""
	local rel_park_name = ""
	while true do
		local rel = NextRelation()
		if not rel then break end
		local rel_boundary = FindInRelation("boundary")
		if rel_boundary=="administrative" then
			isBoundary = true
	...
	-- Landcover (from landuse, natural, leisure)
	local l = landuse
	...
	if park_class=="" and rel_park_class~="" then
		park_class = rel_park_class
	end
	if park_class~="" then
		Layer("park",true)
		SetMinZoomByArea()
		Attribute("class", park_class)
		SetNameAttributes()
		-- If no name from way tags, try relation name
		if not Holds("name") and rel_park_name~="" then
			Attribute("name:latin", rel_park_name)
		end
	end
